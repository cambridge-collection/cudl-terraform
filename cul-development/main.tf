...
module "cudl-data-processing" {
  ...
  efs_subnets                               = zipmap(["${local.base_name_prefix}-subnet-private-eu-west-1a", "${local.base_name_prefix}-subnet-private-eu-west-1b"], module.base_architecture.vpc_private_subnet_ids)
...
module "content_loader" {
  ...
  iam_task_additional_policies = {
    development_releases = aws_iam_policy.development_cudl_data_releases.arn,
    development_source   = aws_iam_policy.development_cudl_data_source.arn,
