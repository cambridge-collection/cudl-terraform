...
When this completes, update the pinned image digests in the target environment:
...
This queries ECR and automatically updates all `sha256:...` digests in
`institution.auto.tfvars`, `terraform.tfvars`, and `scripts/copy-ecr-images.sh`.
...
A bare filename resolves to `scripts/params.json` in the repo root `scripts/` directory.
...
(`eu-west-1`) under `/Environments/Development/CUDL/`:
...
|---|---|---|
| `ContentLoader/DB/Password` | SecureString | PostgreSQL password for the Content Loader database |
| `Services/DB/Password` | SecureString | **Legacy** — set to any placeholder value |
...
| `Viewer/Google/AnalyticsId` | SecureString | **Legacy** — UA was sunset; set to any placeholder |
...
aws ssm put-parameter \
  --name "/Environments/Development/CUDL/ContentLoader/DB/Password" \
  --value "new-password" \
  --type SecureString \
  --overwrite \
  --region eu-west-1
...
| Bucket | Purpose |
|---|---|
| `development-cul-cudl-data-source` | Incoming source data — TEI files, images, collections |
| `development-cul-cudl-data-releases` | Processed output — populated automatically by Lambda |
...
```bash
cd ..
./scripts/load-sample-data.sh --bucket development-cul-cudl-data-source
```
