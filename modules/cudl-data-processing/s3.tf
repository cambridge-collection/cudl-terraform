resource "aws_s3_bucket" "dest-bucket" {
  bucket = substr(lower("${var.environment}-${var.destination-bucket-name}"), 0, 63)
}

resource "aws_s3_bucket" "source-bucket" {
  bucket = substr(lower("${var.environment}-${var.source-bucket-name}"), 0, 63)
}

resource "aws_s3_bucket" "enhancements-bucket" {
  bucket = substr(lower("${var.environment}-${var.enhancements-bucket-name}"), 0, 63)
}
