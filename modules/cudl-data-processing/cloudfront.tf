...
resource "aws_cloudfront_distribution" "transcriptions" {
  ...
  comment      = "${aws_acm_certificate.transcriptions_us-east-1.0.domain_name} CloudFront Distribution"
  ...
  aliases = [
    aws_acm_certificate.transcriptions_us-east-1.0.domain_name
  ]

  origin {
    domain_name              = aws_s3_bucket.dest-bucket.bucket_regional_domain_name
    origin_id                = aws_acm_certificate.transcriptions_us-east-1.0.domain_name
  ...
  default_cache_behavior {
    ...
    target_origin_id       = aws_acm_certificate.transcriptions_us-east-1.0.domain_name
    viewer_protocol_policy = "redirect-to-https"
    cache_policy_id        = data.aws_cloudfront_cache_policy.managed_caching_disabled.id
  }

  viewer_certificate {
    acm_certificate_arn            = aws_acm_certificate.transcriptions_us-east-1.0.arn
