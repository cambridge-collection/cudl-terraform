terraform {
  ...
  backend "s3" {
    ...
    key            = "jlf44-sandbox--content-loader-cudl-infra.tfstate"
...
module "cudl-content-loader" {

  source = "../modules/cudl-content-loader"
  aws-account-number = var.aws-account-number
  environment = var.environment
  source-bucket-name = var.source-bucket-name
  destination-bucket-name = var.destination-bucket-name
  content-loader-domain = var.content-loader-domain
}
